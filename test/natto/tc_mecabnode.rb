...
class TestMeCabNode < Test::Unit::TestCase
  ...
  def setup
    ...
    nm = Natto::MeCab.new('-N2')
    @nb_nodes = []
    #nm.parse(`#{@test_cmd}`) { |n| @nb_nodes << n }
    nm.parse('これは初心です。') {|n| @nb_nodes << n }
  end

  def teardown
    @nodes, @nb_nodes, @host_os, @arch, @test_cmd = nil, nil, nil, nil, nil
  end
  ...
  def test_is_eon
    @nb_nodes.each do |n|
      puts "#{n}"
      if n[:bnext] && n[:bnext].address!=0x0
        foo = lambda do |f|
          nn = Natto::MeCabNode.new(f)
          puts "*** #{nn}"
          foo.call(nn[:bnext]) if nn[:bnext] && nn[:bnext].address!=0x0
        end
        foo.call(n[:bnext])
      end
    end
